class Spree::Admin::ProductImportsController < Spree::Admin::BaseController
  ...
  def sample_csv_import
    begin
      loader = DataShift::SpreeEcom::ProductLoader.new(DATASHIFT_CSV_FILES[:sample_product_file], @options, user: spree_current_user)
      loader.run
  ...
  def user_csv_import
    begin
      pr = ProcessRequest.create(request_file: params[:csv_file],
                                 process_type: 'product_import',
                                 spree_user_id: spree_current_user.id)
      ProductImportJob.perform_later(pr)
      flash[:success] = Spree.t(:successfull_import, scope: :datashift_import, resource: Spree::Product.name.demodulize)
    rescue => e
      flash[:error] = e.message
    end
