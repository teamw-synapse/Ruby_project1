      ...
      class Order
        def self.import(user, params)
          ...
          if completed_at = params.delete(:completed_at)
            ...
            order.approved_at = completed_at
        ...
        def self.create_shipments_from_params(shipments_hash, order)
          ...
          shipments_hash.each do |s|
            ...
            inventory_units.each do |inventory_unit|
              ...
              stock_item = inventory_unit.variant.stock_items.find_by(stock_location_id: shipment.stock_location.id)
              stock_movement = shipment.stock_location.stock_movements.build(quantity: -inventory_unit.quantity, stock_item: stock_item, originator: shipment)
              stock_movement.save!
              inventory_unit.save!
            end
        ...
        def self.create_inventory_units_from_order_and_params(order, inventory_unit_params)
          inventory_unit_params.each_with_object([]) do |inventory_unit_param, inventory_units|
            ...
            inventory_units << InventoryUnit.new(line_item: line_item, order_id: order.id, variant: line_item.variant, quantity: inventory_unit_param[:quantity])
            # end
          end
        end
