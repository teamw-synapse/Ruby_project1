...
module Spree
  class Product < Spree::Base
    ...
    has_many :variants,
             ...
             inverse_of: :product,
             class_name: 'Spree::Variant', autosave: true
    ...
    self.whitelisted_ransackable_attributes = %w[description name slug discontinue_on approved]
    self.whitelisted_ransackable_scopes = %w[not_discontinued approved]

    [
      :sku, :price, :currency, :weight, :height, :width, :depth, :is_master,
      :cost_currency, :price_in, :amount_in, :cost_price, :barcode
    ].each do |method_name|
    ...
    def self.vendor_products(vendor)
      where(vendor_id: vendor.id)
    end

    def commission_price=(commission_price)
      self[:commission_price] =
        case commission_price
        when String
          separator = I18n.t('number.currency.format.separator')
          number    = commission_price.delete("^0-9-#{separator}\.").tr(separator, '.')
