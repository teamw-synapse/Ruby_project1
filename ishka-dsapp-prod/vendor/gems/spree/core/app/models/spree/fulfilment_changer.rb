...
module Spree
  class FulfilmentChanger
    ...
    def update_current_shipment_inventory_units(quantity, state)
      current_shipment_inventory_unit = current_shipment.inventory_units.where(variant: variant).order(state: :asc).first
      desired_shipment_inventory_unit = desired_shipment.inventory_units.where(variant: variant).order(state: :asc).first
      orig_qty = current_shipment_inventory_unit.quantity
      desired_qty = desired_shipment_inventory_unit&.quantity
      if orig_qty == quantity
        if desired_shipment_inventory_unit.present?

          current_shipment_inventory_unit.destroy
          desired_shipment_inventory_unit.update_attributes(quantity: desired_qty + quantity)
        else
          current_shipment_inventory_unit.update_attributes(shipment_id: desired_shipment.id)
        end
      else
        current_shipment_inventory_unit.update_attributes(quantity: orig_qty - quantity)
        if desired_shipment_inventory_unit.present?

          desired_shipment_inventory_unit.update_attribute(:quantity, desired_qty + quantity)
        else
          desired_shipment.inventory_units.create(state: state, quantity: quantity,
                                                  variant: variant, line_item_id: current_shipment_inventory_unit.line_item_id,
                                                  order_id: current_shipment_inventory_unit.order_id)
        end
      end
    end
