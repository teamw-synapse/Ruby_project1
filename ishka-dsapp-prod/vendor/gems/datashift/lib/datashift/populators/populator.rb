...
module DataShift
  ...
  class Populator
    ...
    def prepare_and_assign(context, record, data, user = nil)
      prepare_and_assign_method_binding(context.method_binding, record, data, user)
    end
    ...
    def prepare_and_assign_method_binding(method_binding, record, data, user = nil)
      prepare_data(method_binding, data, user)

      assign(method_binding, record, user)
    end
    ...
    def prepare_data(method_binding, data, user)
    ...
    def assign(method_binding, record, user)
      model_method = method_binding.model_method
      ...
      if model_method.operator_for(:belongs_to)
        ...
        begin
         ...
         record.available_on = Date.today
         record.shipping_category_id = Spree::ShippingCategory.first.id
        if model_method.connection_adapter_column
          if user.vendors.active.any?
            record.vendor_id = user.vendors.active.first.id
          end
