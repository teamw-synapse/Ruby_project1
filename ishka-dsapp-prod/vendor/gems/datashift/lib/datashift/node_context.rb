...
module DataShift
  ...
  class NodeContext
    ...
    attr_accessor :doc_context, :method_binding, :row_index, :user

    attr_accessor :populator

    attr_reader :data

    def initialize(doc_context, method_binding, row_idx, data, user = nil)
    ...
    def process
      populator.prepare_and_assign(self, doc_context.load_object, data, user)
