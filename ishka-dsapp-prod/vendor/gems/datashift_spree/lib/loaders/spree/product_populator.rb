...
module DataShift
  module  SpreeEcom
    class ProductPopulator < Populator
      ...
      def prepare_and_assign_method_binding(method_binding, record, data, user)
        prepare_data(method_binding, data, user)
        ...
        if method_binding.operator?('variants') || method_binding.operator?('option_types')
        ...
        elsif method_binding_for_variant?(method_binding) && !product_load_object.variants.empty?
          if data.to_s.include?(multi_assoc_delim)
            ...
            colmn = method_binding.operator.gsub('variant_', "")
            if record.variants.size == values.size
              record.variants.each_with_index { |v, i| v.send("#{colmn}=", values[i])  }
              record.save
            else
              puts "WARNING: #{method_binding.operator} entries did not match number of Variants - None Set"
            end
          end
          ...
          super(method_binding, product_load_object, data, user) if data
      ...
      def method_binding_for_variant?(method_binding)
        method_binding.operator.in?(%w[variant_sku variant_images variant_images variant_price variant_cost_price variant_weight variant_depth variant_height variant_width variant_commission_price])
      end
      ...
      def build_option_types(option_types)
        ...
        sorted_map = optiontype_vlist_map.sort_by { |_ot, ov| ov.size }.reverse
      ...
      def add_options_variants
        ...
        begin
          ...
          raise ProductLoadError, 'Cannot add OptionTypes/Variants - Save failed on parent Product'
        end
          ...
          lead_ovalues.each do |ovname|
            ...
            sorted_map.each do |ot, ovlist|
              ovlist.each do |ov_for_composite|
                ...
                ).first_or_create(presentation: ov_for_composite.humanize)

                ov_list << ov if ov
              end
            end

            next if ov_list.empty?
      ...
      def add_taxons
        ...
        chain_list.each do |chain|
          ...
          raise DataShift::DataProcessingError, "Could not find or create Taxonomy #{parent_name}" unless parent_taxonomy
      ...
      def add_variants_stock(data)
        ...
        # do we have Variants?
        if !@product_load_object.variants.empty?
          ...
          stock_coh_list.each_with_index do |stock_coh, i|
            ...
            if stock_coh.to_s.include?(name_value_delim)
              ...
              stock_location_name = nil
              variant_count_on_hand = stock_coh.to_i
            end

            logger.info "Setting #{variant_count_on_hand} items for stock location #{stock_location_name}..."

            if !stock_location_name.present? # No Stock Location referenced, fallback to default one...
            ...
            logger.warn stock_location.inspect
        ...
        elsif @product_load_object.variants.empty?
          if data.to_s.include?(multi_assoc_delim)
            ...
            if data.to_s.include?(name_value_delim)
              ...
              stock_location_name = nil
              master_count_on_hand = data.to_i
            end
          end
          if !stock_location_name.present? # No Stock Location referenced, fallback to default one...
        ...
        if !@product_load_object.variants.empty?
          ...
          variants_images_list.each_with_index do |variant_images, i|
            ...
            images.each do |image|
              ...
              if image.match(@spree_uri_regexp)
                ...
                image = begin
                  agent.get(uri)
                          ...
                          raise DataShift::BadUri, "Failed to fetch image from URL #{uri}"
                end
                ...
                begin
                  ...
                  until image.body_io.eof?
                    @current_image_temp_file.write image.body_io.read 16_384
                  end
                  ...
                  raise DataShift::DataProcessingError, "Failed to create Image from URL #{uri}"
                ensure
                ...
                alt_text ||= variants[i].name # ensure alt_text is filled
              ...
              logger.debug attachment.inspect.to_s
          ...
          images.each do |image|
            ...
            if image.match(@spree_uri_regexp)
              ...
              alt_text ||= @product_load_object.master.name # ensure alt_text is filled
